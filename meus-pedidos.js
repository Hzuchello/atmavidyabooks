...
function comTempo(promessa, ms) {
  return Promise.race([
    promessa,
    new Promise((_, reject) => setTimeout(() => reject(new Error('tempo')), ms))
  ]);
}
...
async function carregarPedidos() {
  ...
  if (typeof supabaseClient === 'undefined') {
    container.innerHTML = `<p class="livros-status">Não foi possível conectar ao acervo. Recarregue a página.</p>`;
    return;
  }

  const { data: sessionData } = await comTempo(supabaseClient.auth.getSession(), 12000);
  ...
  const { data: pedidos, error: erroPedidos } = await comTempo(
    supabaseClient.from('pedidos').select('*').order('criado_em', { ascending: false }),
    12000
  );
...
async function iniciarPaginaPedidos() {
  ...
  try {
    ...
    console.error(err);
    if (container) {
      const msg = String(err && err.message) === 'tempo'
        ? 'A consulta demorou demais. Verifique a conexão e recarregue.'
        : 'Não foi possível carregar seus pedidos no momento.';
      container.innerHTML = '<p class="livros-status">' + msg + '</p>';
    }
  }
}
