...
function abrirModalAuth(modo = 'login', contexto = null) {
  ...
  if (contexto === 'checkout') {
    notaEl.textContent = 'Entre ou crie uma conta para finalizar sua compra.';
  } else if (contexto === 'pedidos') {
    notaEl.textContent = 'Entre para ver seus pedidos.';
  } else {
    notaEl.textContent = '';
  }
...
async function enviarFormAuth(event) {
  ...
  if (contextoAuthAtual === 'checkout') {
    abrirCarrinho();
  } else if (contextoAuthAtual === 'pedidos' && typeof carregarPedidos === 'function') {
    carregarPedidos();
  }
  ...
  if (session) {
    container.innerHTML = `
      <div class="nav-auth-logado">
        <a href="meus-pedidos.html" class="nav-link-pedidos">Pedidos</a>
        <div class="nav-sair-bloco">
